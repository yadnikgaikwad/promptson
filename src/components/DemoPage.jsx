...
export default function DemoPage() {
  ...
  useEffect(() => {
    ...
    if (!isDeleting) {
      ...
      if (charIndex > 0) {
        ...
        setIsDeleting(false);
        setTypewriterIndex((typewriterIndex + 1) % TYPEWRITER_TEXTS.length);
      }
    }
  ...
  const handleSend = (e) => {
    ...
    setTimeout(() => {
      setMessages((msgs) => [
        ...msgs,
        { role: 'ai', content: 'This is a demo AI response. Replace this with your AI integration.', time: new Date() }
      ]);
    }, 700);
  };
        ...
        <div className="w-full max-w-3xl flex flex-col gap-2">
          <div className="flex items-end gap-2">
            <textarea
              ...
              maxLength={2000}
            />
            <button type="submit" className="min-h-[48px] px-4 rounded-xl bg-gray-900 text-white font-semibold hover:bg-gray-800 transition-colors shadow-sm flex items-center justify-center">
              <Send size={18} />
            </button>
          </div>
          <div className="flex items-center gap-2 mt-1" ref={dropdownRef}>
            ...
            <button type="button" className="text-xs text-gray-400 hover:text-gray-600">Tone</button>
            <span className="ml-auto text-xs text-gray-400">{input.length}/2000</span>
          </div>
        </div>
      </form>
    </div>
